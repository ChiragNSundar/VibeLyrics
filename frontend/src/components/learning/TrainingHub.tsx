...
const TrainingHub: React.FC = () => {
    ...
    // Sub-tab
    const [activeSection, setActiveSection] = useState<'overview' | 'export' | 'import' | 'lmstudio'>('overview');
    ...
    const handleGenerate = async () => {
        ...
        setStatusMsg('');
        try {
            const data = await trainingApi.generate();
            setStats(data);
            setStatusMsg(`✅ Generated ${data.total_pairs} training pairs!`);
    ...
    const handleExport = async (format: string) => {
        try {
            ...
            link.href = url;
            link.download = `vibelyrics_training_data.${format === 'zip' ? 'zip' : format === 'alpaca' ? 'json' : format === 'jsonl' ? 'jsonl' : 'txt'}`;
    ...
    const handleConfigChange = async (key: string, value: string | number) => {
        if (!config) return;
    ...
    return (
        <div className="training-hub">
            ...
            <div className="training-tabs">
                <button
                    className={`training-tab ${activeSection === 'overview' ? 'active' : ''}`}
                    onClick={() => setActiveSection('overview')}
                >
                    📊 Overview
                </button>
                <button
                    className={`training-tab ${activeSection === 'export' ? 'active' : ''}`}
                    onClick={() => { setActiveSection('export'); handlePreview(); }}
                >
                    📤 Export
                </button>
                <button
                    className={`training-tab ${activeSection === 'import' ? 'active' : ''}`}
                    onClick={() => setActiveSection('import')}
                >
                    📥 Import
                </button>
                <button
                    className={`training-tab ${activeSection === 'lmstudio' ? 'active' : ''}`}
                    onClick={() => { setActiveSection('lmstudio'); fetchLMStudioInfo(); }}
                >
                    🧪 LM Studio
                </button>
            </div>
                ...
                <div className="training-section">
                    ...
                    <div className="stats-grid-training">
                        <div className="stat-card">
                            ...
                            <div className="stat-label-training">Total Training Pairs</div>
                        </div>
                        <div className="stat-card">
            ...
            {activeSection === 'import' && (
                <div className="training-section">
                    ...
                    <p className="hint">
                        Import an external dataset in Alpaca JSON or ChatML JSONL format to merge with your learned data.
                    </p>
                    ...
                    <div className="import-info">
                        ...
                        <div className="format-examples">
                            <div className="format-example">
                                <strong>Alpaca JSON:</strong>
                                <pre>{`[
  {
    "instruction": "Write a rap line about...",
    "input": "Previous line here",
    "output": "Generated line here"
  }
]`}</pre>
                            </div>
                            <div className="format-example">
                                <strong>ChatML JSONL:</strong>
                                <pre>{`{"messages": [{"role": "user", "content": "..."}, {"role": "assistant", "content": "..."}]}`}</pre>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {activeSection === 'lmstudio' && (
                <div className="training-section">
                    ...
                    <div className="lm-server-status">
                        ...
                        <span>
                            LM Studio Server: {serverAvailable ? 'Online' : 'Offline'}
                        </span>
                        {serverAvailable && (
                            <span className="model-count">{models.length} model(s) loaded</span>
                        )}
                    </div>
                    ...
                    {config && (
                        <div className="training-config">
                            ...
                            <div className="config-grid">
                                ...
                                <div className="config-field">
                                    <label>Gradient Accum Steps</label>
                                    <input
                                        type="number"
                    ...
                    <div className="pipeline-control">
                        ...
                        <p className="hint">
                            Generates a training script using Unsloth for QLoRA fine-tuning.
                            The script converts the result to GGUF for LM Studio.
                        </p>
                        ...
                        >
                            {startingTraining ? '⏳ Preparing...' : '🚀 Generate Training Script'}
                        </button>
